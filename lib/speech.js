...
function startWebSpeech(onChunk) {
  ...
  recognition.onerror = (e) => {
    ...
    if (e.error === 'not-allowed') {
      if (activeCallback) activeCallback({ text: '', isFinal: false, speaker: null, error: 'マイクへのアクセスが拒否されました。ブラウザの設定でマイクを許可してください。' });
      return;
    }
...
async function startMediaSTT(onChunk) {
  ...
  try {
    ...
    const msg = e.name === 'NotAllowedError'
      ...
      : `マイクを起動できませんでした: ${e.message}`;
    onChunk({ text: '', isFinal: false, speaker: null, error: msg });
    return;
  }
